...
#define GVN_CELLDBLCLICKED      LVN_BEGINLABELEDIT+3
#define GVN_ROWLABELDBLCLICKED      LVN_BEGINLABELEDIT+4
#define GVN_COLUMNLABELDBLCLICKED      LVN_BEGINLABELEDIT+5
...
public:
    int  InsertColumn(LPCTSTR strHeading, UINT nFormat = DT_CENTER|DT_VCENTER|DT_SINGLELINE,
                      int nColumn = -1);
    int  InsertRow(LPCTSTR strHeading, UINT nFormat = DT_CENTER|DT_VCENTER|DT_SINGLELINE,int nRow = -1);
...
protected:
    ...
    int         m_SortColumn;
public:
	void EnlargeFont(int enx=1,int eny=1,int enw=100);
	BOOL locateRow(CString rawTitle,int& rawno);
	BOOL locateColumn(CString colTitle,int& colno);
	void AllowPopupMenu(BOOL allow);
	void OnAddRow();
