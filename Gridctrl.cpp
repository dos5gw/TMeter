...
#define SELECTED_CELL_FONT_WEIGHT 300    // wieght of text for selected items
...
CGridCtrl::CGridCtrl(int nRows, int nCols, int nFixedRows, int nFixedCols)
{
    ...
    m_crWindowColour     = RGB(250,255,255);//::GetSysColor(COLOR_WINDOW);
    m_cr3DFace           = ::GetSysColor(COLOR_3DFACE);
    m_crShadow           = RGB(200,200,200);//::GetSysColor(COLOR_3DSHADOW);
...
CGridCtrl::~CGridCtrl() {
	...
	if (m_bMustUninitOLE) ::OleUninitialize();
	}
...
BOOL CGridCtrl::Create(const RECT& rect, CWnd* pParentWnd, UINT nID, DWORD dwStyle) {
    ...
    if (!CWnd::Create(GRIDCTRL_CLASSNAME, NULL, dwStyle, rect, pParentWnd, nID)) return FALSE;
#ifdef GRIDCONTROL_USE_OLE_DRAGDROP
...
BEGIN_MESSAGE_MAP(CGridCtrl, CWnd)
	...
	ON_WM_RBUTTONDOWN()
	//ON_COMMAND(ID_DELGRIDROW, OnDelRow)
...
void CGridCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
    ...
    switch (nChar)
    {
        case VK_DELETE: 
            if (IsEditable() &&
                !(GetItemState(m_idCurrentCell.row, m_idCurrentCell.col) & GVIS_READONLY))
            {
		...
		case VK_RETURN:
			if (next.row < (GetRowCount() -1))
				next.row ++;
			else if (next.col < (this->GetColumnCount() -1))
			{
				next.col ++;
				next.row = 1;
			}
...
{
    CCellRange Selection = GetSelectedCellRange();
    pCmdUI->Enable(m_bEditable && Selection.Count() && IsValid(Selection));
}
...
void CGridCtrl::OnUpdateEditPaste(CCmdUI* pCmdUI) 
{
    ...
    BOOL bCanPaste = m_bEditable && IsValid(cell) && 
                     (GetItemState(cell.row, cell.col) & GVIS_READONLY == GVIS_READONLY) &&
                            ...
                            int nColumn  /* = -1 */)
{
    ...
    if (!IsEditable() || (nColumn >= 0 && nColumn >= GetColumnCount()))
        return -1;
...
int CGridCtrl::InsertRow(LPCTSTR strHeading,UINT nFormat ,int nRow /* = -1 */)
{
    // If the insertion is for a specific row, check it's within range.
    if (!IsEditable() || (nRow >= 0 && nRow >= GetRowCount())) 
        return -1;
	...
	for (col = 0; col < m_nCols; col++) {
        SetItemFormat(nRow,col, nFormat);
		}
...
CGridCell* CGridCtrl::CreateCell(int nRow, int nCol) {
    ...
    if (nRow > 0 && nCol>0 ) pCell->nFormat = GetItemFormat(nRow-1, nCol);
...
BOOL CGridCtrl::DeleteColumn(int nColumn) {
    if (/*!IsEditable() ||*/ nColumn < 0 || nColumn >= GetColumnCount()) return FALSE;
    ResetSelectedRange();
    for (int row = 0; row < GetRowCount(); row++) {
        GRID_ROW* pRow = m_RowData[row];
...
CCellID CGridCtrl::GetNextItem(CCellID& cell, int nFlags) const
{
    if ((nFlags & GVNI_BELOW) && (nFlags & GVNI_TORIGHT))
    {
		...
		for (; row < GetRowCount(); row++)	
            ...
            {
				int nState = GetItemState(row, col);
				if (nFlags & GVNI_DROPHILITED && nState & GVIS_DROPHILITED) return CCellID(row, col);
				if (nFlags & GVNI_FOCUSED && nState & GVIS_FOCUSED)     return CCellID(row, col);
				if (nFlags & GVNI_SELECTED && nState & GVIS_SELECTED)   return CCellID(row, col);
				if (nFlags & GVNI_READONLY && nState & GVIS_READONLY)   return CCellID(row, col);
...
void CGridCtrl::AutoSize() {
    ...
    for (int nRow = 0; nRow < nNumRows; nRow++) m_arRowHeights[nRow] = 0;
    CSize size;
...
void CGridCtrl::ExpandColumnsToFit() {
	...
	if(nDifference<0) return;
    int nColumnAdjustment = nDifference / GetColumnCount();
    for (int i = 0; i < GetColumnCount(); i++) m_arColWidths[i] += nColumnAdjustment;
    if (nDifference > 0) {
        int leftOver = nDifference % GetColumnCount();
        for (i = 0; i < leftOver; i++) m_arColWidths[i] += 1;
		} 
    else {
        int leftOver = (-nDifference) % GetColumnCount();
        for (i = 0; i < leftOver; i++) m_arColWidths[i] -= 1;
...
void CGridCtrl::ExpandRowsToFit() {
    ...
    for (int i = 0; i < GetRowCount(); i++) m_arRowHeights[i] += nRowAdjustment;
    if (nDifference > 0) {
        int leftOver = nDifference % GetRowCount();
        for (i = 0; i < leftOver; i++) m_arRowHeights[i] += 1;
		} 
	else {
        int leftOver = (-nDifference) % GetRowCount();
        for (i = 0; i < leftOver; i++) m_arRowHeights[i] -= 1;
...
void CGridCtrl::OnLButtonDblClk(UINT nFlags, CPoint point) {
	if (m_MouseMode == MOUSE_NOTHING) {
        if(IsValid(m_LeftClickDownCell)) {
			if (m_LeftClickDownCell.row >= m_nFixedRows && m_LeftClickDownCell.col >= m_nFixedCols) {
				SendMessageToParent(m_idCurrentCell.row, m_idCurrentCell.col, GVN_CELLDBLCLICKED);
				}
			else {
				if(m_LeftClickDownCell.row < m_nFixedRows) SendMessageToParent(m_LeftClickDownCell.row, m_LeftClickDownCell.col, GVN_ROWLABELDBLCLICKED);
				else SendMessageToParent(m_LeftClickDownCell.row, m_LeftClickDownCell.col, GVN_COLUMNLABELDBLCLICKED);
				}
			}
		}
...
void CGridCtrl::PrintFooter(CDC *pDC, CPrintInfo *pInfo)
{
    ...
    CTime t = CTime::GetCurrentTime();//::GetTimeFromDog();;
    sTemp = t.Format(_T("%c"));
...
CSize CGridCtrl::GetTextExtent(LPCTSTR str, BOOL bUseSelectedFont /* = TRUE */)
{
    ...
    return size + CSize(m_nMargin, m_nMargin);
}
...
CSize CGridCtrl::GetCellExtent(int nRow, int nCol, CDC* pDC)
{
	...
	CString itemText=GetItemText(nRow, nCol);
    CSize size = pDC->GetTextExtent(itemText);
    pDC->SelectObject(pOldFont);

    size += CSize((m_nMargin+2)*2,m_nMargin);
...
void CGridCtrl::Serialize(CArchive& ar) {
	int col,row,rowNo,colNo,fixRowNo,fixColNo;
	COLORREF color;
	GV_ITEM Item;
	Item.mask = GVIF_TEXT|GVIF_FORMAT|GVIF_FGCLR;
	Item.nFormat = DT_CENTER|DT_VCENTER|DT_SINGLELINE|DT_END_ELLIPSIS;
	CString cellContent;
	if(ar.IsLoading()) {
		ar>>rowNo>>colNo;
		ar>>fixRowNo>>fixColNo;
		SetColumnCount(colNo);
		SetRowCount(rowNo);
		SetFixedColumnCount(fixRowNo);
		SetFixedRowCount(fixColNo);
		for(row=0;row<rowNo;row++) for(col=0;col<colNo;col++) {
			Item.row=row;Item.col=col;
			ar>>cellContent;ar>>color;
			Item.crFgClr=color;
			Item.szText=cellContent;
			SetItem(&Item);
			}	
		}
	else  {
		ar<<m_nRows<<m_nCols;
		ar<<m_nFixedRows<<m_nFixedCols;
		for(row=0;row<m_nRows;row++) for(col=0;col<m_nCols;col++) {
			ar<<GetItemText(row,col)<<GetItemFgColour(row,col);
			}	
		}
 	}

void CGridCtrl::OnRButtonDown(UINT nFlags, CPoint point) {
	if(m_bAllowPopupMenu) {
		/*
		...
		if(cell.row>=GetFixedRowCount()&&cell.col>=GetFixedColumnCount()) {
			...
			menu.AppendMenu(0, ID_DELGRIDROW, "É¾³ýµ±Ç°ÐÐ");
			menu.TrackPopupMenu(TPM_LEFTALIGN,point.x,point.y,this);
			}*/
		}
	}

void CGridCtrl::OnDelRow() {
...
void CGridCtrl::AllowPopupMenu(BOOL allow) {
	m_bAllowPopupMenu=allow;
	}

BOOL CGridCtrl::locateRow(CString rawTitle, int &rawno) {
	for(rawno=1;rawno<GetRowCount();rawno++) {
		if(rawTitle==GetItemText(rawno,0)) return TRUE;
		}
	return FALSE;
	}

BOOL CGridCtrl::locateColumn(CString colTitle, int &colno) {
	for(colno=1;colno<GetColumnCount();colno++) {
		if(colTitle==GetItemText(0,colno)) return TRUE;
		}
	return FALSE;
	}
