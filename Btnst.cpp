...
//pDC->FillRect(&itemRect, &br);
  ...
  else // ...else draw non pressed button
  {
    ...
    if (m_bIsFlat == TRUE)
    {
      if (m_MouseOnButton == TRUE && m_bDrawBorder == TRUE) {
		...
		pOldPen = pDC->SelectObject(&pen3DDKShadow);
		pDC->MoveTo(itemRect.left, itemRect.bottom-1);
		pDC->LineTo(itemRect.right-1, itemRect.bottom-1);
		...
		pDC->LineTo(itemRect.right-2, itemRect.top+1);
		pDC->LineTo(itemRect.left+1, itemRect.top+1);
		pDC->LineTo(itemRect.left+1, itemRect.bottom-2);
  ...
  if (sTitle.IsEmpty() == FALSE)
  {
    ...
    pDC->DrawText(sTitle, -1, captionRect, DT_SINGLELINE|DT_CALCRECT); //@ 2012-06-21 !original
	//pDC->DrawText(sTitle, -1, captionRect, DT_EDITCONTROL | DT_WORDBREAK | DT_CENTER);
	//pDC->DrawText(sTitle, -1, captionRect, DT_WORDBREAK);
...
void CButtonST::DrawTheIcon(CDC* pDC, CString* title, RECT* rcItem, CRect* captionRect, BOOL IsPressed, BOOL IsDisabled) {
  CRect iconRect = rcItem;int vertGap;
  switch (m_nAlign) {
    case ST_ALIGN_HORIZ:
         if (title->IsEmpty()) {
			...
			iconRect.left+=(vertGap=((rcItem->right-rcItem->left-m_cxIcon-titleSize.cx)/2));
			captionRect->left = m_cyIcon+vertGap;
			}
    ...
    case ST_ALIGN_VERT:
         ...
         if (title->IsEmpty()) {
			...
			CSize titleSize=pDC->GetTextExtent(*title);
			iconRect.top+=(vertGap=((rcItem->bottom-rcItem->top-m_cyIcon-titleSize.cy)/2));
			captionRect->top = m_cyIcon+vertGap;
			}
         break;
	}
